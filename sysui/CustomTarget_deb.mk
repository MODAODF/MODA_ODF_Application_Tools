...
$(foreach product,$(PRODUCTLIST),\
	...
	$(product)$(PKGVERSIONSHORT)-debian-menus_$(PRODUCTVERSION)-$(OXO_VERSION_PATCH)_all.deb) \
))

$(deb_WORKDIR)/%-desktop-integration.tar.gz: $(deb_WORKDIR)/%$(PKGVERSIONSHORT)-debian-menus_$(PRODUCTVERSION)-$(OXO_VERSION_PATCH)_all.deb
...
$(deb_WORKDIR)/%/DEBIAN/control: $(deb_SRCDIR)/control $(call gb_CustomTarget_get_workdir,sysui/share)/%/create_tree.sh
	...
	cat $< | tr -d "\015" | \
		...
		>> $@
	echo "Version: $(PRODUCTVERSION)-$(OXO_VERSION_PATCH)" >>$@
	du -k -s $(deb_WORKDIR)/$* | $(gb_AWK) -F ' ' '{ printf "Installed-Size: %s\n", $$1 ; }' >>$@

$(deb_WORKDIR)/%$(PKGVERSIONSHORT)-debian-menus_$(PRODUCTVERSION)-$(OXO_VERSION_PATCH)_all.deb: $(deb_WORKDIR)/%/DEBIAN/postrm $(deb_WORKDIR)/%/DEBIAN/postinst $(deb_WORKDIR)/%/DEBIAN/prerm $(deb_WORKDIR)/%/DEBIAN/control
